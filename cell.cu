...
#define stopbeat 1500
#define time_before_beat 100 //ms

#define out_step	40 // number of steps to output data
...
#define Vi (1.0/Nci)	//um^3, Volume of the cytosolic space, for each compartment
...
#define tauil 0.49	//ms, diffusion time of longitudinal cytosolic
#define tauit 0.231 	//ms, diffusion time of transverse cytosolic
#define	Datp 	2.0	// diffusion time prefactor of ATP
#define Ddye 	2.0	// diffusion time prefactor of dye
#define Degta 	4.0	// diffusion time prefactor of EGTA
...
// #define	gKs  (atof(argv[6]))  // mS/uF, IKs conductance
#define gtof (0.1)  // mS/uF, Itof conductance
#define gtos (0.04)  // mS/uF, Itos conductance
// #define gNaK (1.5*0.9)  // mS/uF, INaK conductance
#define gK1  (0.6)  // mS/uF, IK1 conductance
#define gNa  (12.0)  // mS/uF, INa conductance
#define gNaLeak (0.00105)  // mS/uF, INaLeak conductance
// #define gNaL 0.01
#define Vncx 	(21.0) //uM/ms, strength of NCX current
...
#define tauu 1100.0  // ms, transition rate from CSQN-bound states to CSQN-unbound states
...
// LCC ica
#define Pca (10.0)	// umol/C/ms, 11.9: Restrepo 2008
...
#define kegtaon 	0.004 // EGTA
#define kegtaoff	0.002
...
double Inal(double v, double *hl, double *ml, double nai, double alpha_NaL, double gNaL);
double Ikr( double v, double *Xkr );
double Iks( double v, double *Xs1, double *Xs2, double *Qks, double cst, double nai, double gKs);
double Ik1( double v );
double Itos(double v, double *Xtos, double *Ytos);
double Itof(double v, double *Xtof, double *Ytof);
double Inak( double v, double nai, double gNaK );
double sodium(double v, double nai, double I_Na, double I_NaL, double I_NaK, double I_NCX);
...
int main(int argc, char **argv)
{
	...
	while ( step*DT < stoptime )
	{
		...
		I_NaL = Inal(v, &hl, &ml, nai, atof(argv[4]), atof(argv[8]));
		I_Kr = Ikr(v, &Xkr);
		I_Ks = Iks(v, &Xs1, &Xs2, &Qks, cst, nai, atof(argv[6]) );
		I_K1 = Ik1(v);
		I_tos = Itos(v, &Xtos, &Ytos);
		I_tof = Itof(v, &Xtof, &Ytof);
		I_NaK = Inak(v, nai, atof(argv[7]));
		nai = sodium(v, nai, I_Na, I_NaL, I_NaK, I_NCX);

		///////////////////////////	Action Potential ///////////////////////////////////
		double stim = 0;
		if( fmod(step*DT+PCL-time_before_beat,PCL) < 1.0 && step*DT > time_before_beat && step*DT < PCL*stopbeat )
			stim = 80.0;
		double dvh = -( I_Na + I_NaL + I_K1 + I_Kr + I_Ks + I_tos + I_tof + I_NCX + I_Ca + I_NaK + I_bg ) + stim; 
		v += dvh*DT;
				...
				for (i=0;i<Nx;i++)
				{
					for ( ix=0;ix<Nix;ix++)
					{
						fprintf(fluoxt_file,"%g\t%g\t%g\t%g\t%g\n",
								...
								fluo[posall(i*Nix+ix, Ny/2*Niy+0, 4*Niz+0)], fluo[posall(i*Nix+ix,Ny/2*Niy+0,5*Niz+0)], 
								fluo[posall(i*Nix+ix,(Ny/2+1)*Niy+0,6*Niz+0)]
...
__global__ void Compute( cru *CRU, cru2 *CRU2, cytosol *CYT, cyt_bu *CBU, sl_bu *SBU, double v, int step, double nai)
{
	...
	if ((i*j*k)!=0 && i<Nx-1 && j<Ny-1 && k<Nz-1)
	{
		...
		for( int iii = 0; iii < FINESTEP; ++iii )
		{
			...
			double csdiffatp = ( SBU[pos(i,j,k+1)].caatp+SBU[pos(i,j,k-1)].caatp-2*SBU[ps].caatp )/(Datp*taust);
			double csdiffdye = ( SBU[pos(i,j,k+1)].cadye+SBU[pos(i,j,k-1)].cadye-2*SBU[ps].cadye )/(Ddye*taust);
			double csdiffegta = ( SBU[pos(i,j,k+1)].caegta+SBU[pos(i,j,k-1)].caegta-2*SBU[ps].caegta )/(Degta*taust);
...
__device__ int ryrgating(double cp, double cjsr, curandState *state, int *ncu, int *nou, int *ncb, int *nob, int i, int j, int k, int step)
{
	...
	double ku = 80.0/( 1.0 + pow2(5000.0/cjsr) )/( 1.0 + pow2(19.0/cp) );
	double kb = 35.56/( 1.0 + pow2(5000.0/cjsr) )/( 1.0 + pow2(19.0/cp) );
...
{	
	...
	double tauf_inverse = (0.02-0.007*exp(-pow2(0.0337*(v+10.5))))/1.2;
...
double Inal(double v, double *hl, double *ml, double nai, double alpha_NaL, double gNaL)
{
	...
	double Vom = 37.5682 + alpha_NaL*15.1598;
	double Vdm = 35.1861 - alpha_NaL*10.8591;
	double Voh = -16.9029 + alpha_NaL*7.6313;
	double Vdh = 12.0 + alpha_NaL*7.913;
	double ml_inf = 1.0/( 1.0 + exp( -(v+Vom)/Vdm) );
	double hl_inf = 1.0/( 1.0 + exp( (v+Voh)/Vdh ) );

	*hl = hl_inf;
	*ml = ml_inf;

	return (gNaL*(*ml)*(*ml)*(*ml)*(*hl)*(*hl)*(v-Ena));
}
...
double Inak( double v, double nai, double gNaK )	 // Mahajan et al 2008
{
...
double sodium(double v, double nai, double I_Na, double I_NaL, double I_NaK, double I_NCX)
{
	...
	double dnai = - trick * alpha * ( I_Na + I_NaL + 3.0*I_NaK + 3.0*I_NCX );
